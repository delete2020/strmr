...
/**
 * Route handler for external player error callback
 * Called when playback fails in an external player like Outplayer or Infuse
 */
export default function PlaybackDidFail() {
  ...
  useEffect(() => {
    ...
    Linking.getInitialURL().then((url) => {
      console.log('=== EXTERNAL PLAYER CALLBACK (ERROR) ===');
      ...
      // Log common error parameters
      const allParams = { ...globalParams, ...localParams };
      if (allParams.errorCode !== undefined) {
        console.log('Error code:', allParams.errorCode);
      }
      if (allParams.errorMessage !== undefined) {
        console.log('Error message:', allParams.errorMessage);
      }
      if (allParams.error !== undefined) {
    ...
    const subscription = Linking.addEventListener('url', (event) => {
      console.log('=== URL EVENT RECEIVED (ERROR) ===');
