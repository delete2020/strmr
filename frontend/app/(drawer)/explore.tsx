...
export default function MoviesScreen() {
  ...
  const { data: trendingMovies, loading, error } = useTrendingMovies();

  const filteredMovies = useMemo(() => {
    const titles = trendingMovies?.map((item) => item.title) ?? [];
    return titles.filter((item) => matchesYearRange(item.year, preferences.yearRange));
  }, [preferences.yearRange, trendingMovies]);
  ...
  return (
    <SpatialNavigationRoot isActive={isActive} onDirectionHandledWithoutMovement={onDirectionHandledWithoutMovement}>
      ...
      <View style={styles.container}>
        ...
        <MediaGrid
          title={`Trending Movies • ${qualityLabel}`}
