 ...
 * These should be avoided unless specifically requested or they're the only option.
 */
const isAudioDescriptionTrack = (title: string | undefined): boolean => {
  ...
  const adPatterns = [
    ...
    'dvs',
    ' ad)',  // Space before to avoid matching "audio"
    '(ad)',
    '[ad]',
    ' ad ',
    '-ad-',
...
/**
 * Combined check for tracks that should be avoided in normal playback.
 */
...
export const findAudioTrackByLanguage = (streams: AudioStreamMetadata[], preferredLanguage: string): number | null => {
  ...
  // Pass 2: Non-TrueHD codec matching language, skipping commentary/AD
  ...
  // Pass 5: Non-TrueHD codec matching language, including commentary/AD (fallback)
  for (const stream of streams) {
