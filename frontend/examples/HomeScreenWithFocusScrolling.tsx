...
export const HomeScreenWithFocusScrolling = () => {
  ...
  return (
    <SpatialNavigationRoot isActive={true}>
      <FocusScrollView ref={scrollViewRef} style={styles.container} contentContainerStyle={styles.content}>
        {menuItems.map((item) => (
          <FocusableItem
            key={item.id}
            itemKey={item.id}
            ...
            {({ isFocused }) => (
              <View style={[styles.itemContent, isFocused && styles.itemFocused]}>
                <Text style={[styles.itemText, isFocused && styles.itemTextFocused]}>{item.title}</Text>
              </View>
            )}
          </FocusableItem>
        ))}
