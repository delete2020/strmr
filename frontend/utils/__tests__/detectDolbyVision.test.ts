...
describe('hasDolbyVisionTag', () => {
  it('identifies explicit Dolby Vision tags', () => {
    expect(
      hasDolbyVisionTag('Furiosa.2024.2160p.WEB-DL.DOLBY.VISION.DV.HDR.Atmos.mkv'),
    ).toBe(true);
  ...
  it('supports multiple candidates', () => {
    expect(
      hasDolbyVisionTag(
        'Series.S01E01.2160p.WEB.HDR10Plus.x265',
        'Series.S01E01.2160p.WEB.DV.HDR10Plus.x265',
      ),
    ).toBe(true);
