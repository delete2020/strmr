...
func (h *VideoHandler) StartHLSSession(w http.ResponseWriter, r *http.Request) {
	...
	session, err := h.hlsManager.CreateSession(r.Context(), cleanPath, path, hasDV, dvProfile, hasHDR, forceAAC, startSeconds, transcodingOffset, audioTrackIndex, subtitleTrackIndex, profileID, profileName, getClientIP(r), r.UserAgent())
	if err != nil {
