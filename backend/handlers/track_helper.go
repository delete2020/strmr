...
func FindAudioTrackByLanguage(streams []AudioStreamInfo, preferredLanguage string) int {
	...
	// Pass 2: Non-TrueHD incompatible codec (DTS, etc.) matching language, skipping commentary and audio description
	...
	// Pass 5: Non-TrueHD incompatible codec matching language, including commentary and audio description (fallback)
	for _, stream := range streams {
		if matchesLanguage(stream.Language, stream.Title, normalizedPref) &&
			!IsTrueHDCodec(stream.Codec) {
			log.Printf("[track] ⚠️ Fallback to commentary/AD track: %d (%s) for language %q - will need HLS transcoding",
				stream.Index, stream.Codec, preferredLanguage)
			return stream.Index
		}
	}
	...
	for _, stream := range streams {
		if matchesLanguage(stream.Language, stream.Title, normalizedPref) {
			log.Printf("[track] ⚠️ Fallback to commentary/AD track: %d (%s) for language %q (only option) - will need HLS transcoding",
				stream.Index, stream.Codec, preferredLanguage)
			return stream.Index
		}
	}
